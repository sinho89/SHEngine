#include "Samples.h"
#include "ixrEngine/ixrEngineMinimal.h"
#ifdef USE_AWS_MODULE
...
VOID onDataChannelMessage(UINT64 customData, PRtcDataChannel pDataChannel, BOOL isBinary, PBYTE pMessage, UINT32 pMessageLen)
{
    ...
    if (isBinary)
    {
        ...
        if (pSampleStreamingSession->pSampleConfiguration)
        {
            if (pSampleStreamingSession->pSampleConfiguration->AWSKinesisVideoWebRTCComponent)
            {
                pSampleStreamingSession->pSampleConfiguration->AWSKinesisVideoWebRTCComponent->OnInputData(pMessage, pMessageLen);
            }
        }
    }
    /*else 
    ...
    retStatus = dataChannelSend(pDataChannel, FALSE, (PBYTE) MASTER_DATA_CHANNEL_MESSAGE, (UINT32)STRLEN(MASTER_DATA_CHANNEL_MESSAGE));
...
STATUS handleOffer(PSampleConfiguration pSampleConfiguration, PSampleStreamingSession pSampleStreamingSession, PSignalingMessage pSignalingMessage)
{
    ...
    //Sleep(2000); // VR에서 너무 빨리 응답하면 안붙어서..
                                    ...
                                    PSampleStreamingSession* ppSampleStreamingSession)
{
    ...
    CHK_STATUS(addSupportedCodec(pSampleStreamingSession->pPeerConnection, RTC_CODEC_ALAW));// RTC_CODEC_OPUS));
    CHK_STATUS(addSupportedCodec(pSampleStreamingSession->pPeerConnection, RTC_CODEC_OPUS));

    // Add a Send Transceiver of type video
    ...
    // Add a Send Transceiver of type audio
...
STATUS freeSampleStreamingSession(PSampleStreamingSession* ppSampleStreamingSession)
{
    ...
    if (pSampleConfiguration->iceCandidatePairStatsTimerId != MAX_UINT32 && pSampleConfiguration->streamingSessionCount == 0 &&
        ...
        //CHK_LOG_ERR(timerQueueCancelTimer(pSampleConfiguration->timerQueueHandle, pSampleConfiguration->iceCandidatePairStatsTimerId, (UINT64) pSampleConfiguration));
                                            ...
                                            UINT64 duration)
{
    ...
    if (percentLost < 2) {
        ...
        bitrate = (UINT32)(bitrate * 1.02f);
    } else if (percentLost > 5) {
        // decrease encoder bitrate by packet loss percent
        bitrate = (UINT32)(bitrate * (1.0f - (float)percentLost / 100.0f));
    }
...
STATUS lookForSslCert(PSampleConfiguration* ppSampleConfiguration)
{
    ...
    if (pSampleConfiguration->pCaCertPath == NULL) {
        ...
        CHK(0 == FSTAT(pSampleConfiguration->pCaCertPath, (struct _stat64i32*) & pathStat), STATUS_DIRECTORY_ENTRY_STAT_ERROR);
...
STATUS createSampleConfiguration(PCHAR channelName, PCHAR RegionName, SIGNALING_CHANNEL_ROLE_TYPE roleType, BOOL trickleIce, BOOL useTurn,
                                 PSampleConfiguration* ppSampleConfiguration)
{
    ...
    //CHK(NULL != (pSampleConfiguration = (PSampleConfiguration) malloc(SIZEOF(SampleConfiguration))), STATUS_NOT_ENOUGH_MEMORY);
    ...
    {
        pSampleConfiguration->channelInfo.pRegion = RegionName;
    }
...
STATUS CheckSessionCleanup(PSampleConfiguration pSampleConfiguration)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PSampleStreamingSession pSampleStreamingSession = NULL;
    ...
    if (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->interrupted)) 
    {
        ...
        CVAR_WAIT(pSampleConfiguration->cvar, pSampleConfiguration->sampleConfigurationObjLock, 0);
        MUTEX_UNLOCK(pSampleConfiguration->sampleConfigurationObjLock);
        locked = FALSE;
    }
